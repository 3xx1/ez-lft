...
IF /I "%arg1%" EQU "build" (
  ...
  docker run --rm --name ez-lft -i -t -p 80:80 -v "%cd%/:/ez-lft" -e NODE_ENV=development -e NODE_PORT=80 -e ORIGIN=http://localhost:80 kk/ez-lft:latest /bin/bash -c "python diagnose.py"
) ELSE (
